...
> 추가로, CodeRabbit 등의 AI 리뷰어가 지적한 사항을 수정하여 커밋할 때, 커밋 메시지에 "CodeRabbit이 시켜서 했다"와 같은 뉘앙스나 문구를 절대 포함하지 않아야 합니다. (예: `fix: 요구사항 수정` 형태로 깔끔하게 작성)
> 또한 커밋 내역, PR내역은 항상 한글로 작성한다 접두사면 영어로 작성  
## 1. 코드 및 프로젝트 구조 컨벤션
